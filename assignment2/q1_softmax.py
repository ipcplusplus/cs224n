...
def softmax(x):
    ...
    #softmax = ( e ^ x ) / (sum e^i), max to be reduced each time 
    
    max_value = tf.reduce_max(x, axis = 1, keep_dims = True)
    new_x = tf.subtract(x, max_value)
    x_exp = tf.exp(new_x)
    total_sum = tf.reduce_sum(x_exp, axis = 1, keep_dims = True)
    out = x_exp / total_sum
...
def cross_entropy_loss(y, yhat):
    ...
    ### YOUR CODE HERE
    ### END YOUR CODE
    #cross - entropy, or the loss function: -sum *  [y(i) * log{y(i`)}]
    
    log_yhat = tf.log(yhat)
    comb = tf.multiply(tf.to_float(y), log_yhat)
    summ = tf.reduce_sum(comb, axis = 1, keepdims = True)
    neg = tf.negative(summ)
    out = tf.reduce_sum(neg)
    return out
